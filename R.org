...
#+BEGIN_SRC R :session :exports both :results output graphics :file boxplot1.png
  ...
  add.outlier <- function(p,labvar = as.character(p$mapping$y)){
      ...
      mm <- Reduce(rbind,tapply(y,x,FUN = function(y){
                              data.frame(y = y[y <= (fivenum(y)[2] - 1.5 * IQR(y)) | y >= (fivenum(y)[4] + 1.5 * IQR(y))],
                                         x = x[y <= (fivenum(y)[2] - 1.5 * IQR(y)) | y >= (fivenum(y)[4] + 1.5 * IQR(y))])}))
