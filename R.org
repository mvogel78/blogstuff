...
#+BEGIN_SRC R :session :exports both :results output graphics :file boxplot1.png
  ...
  p <- ggplot(mtcars,aes(x=cyl,colour=cyl,y=mpg)) +
      geom_boxplot()
  ...
  data <- data.frame(x = factor(sample(1:10,1000,replace = T)),y = rnorm(1000))
  data <- data.frame(x = sample(1:10,1000,replace = T),y = rnorm(1000))
  
  
  p <- ggplot(data,aes(x=x,colour=x,y=y)) +
      geom_boxplot()
  ...
  p <- ggplot(data,aes(x=timepoint,colour=timepoint,y=STAI_score)) +
      geom_boxplot()
  add.outlier(p, "ID")
